...
from .. import tp_pdf_cens as tpc

TOL = 1e-3


def test_t_peak_cens_fitter():
    t_0 = 13.8
    x_target = np.linspace(1, t_0, 100)
    pdf_target = np.ones_like(x_target)
    args = (x_target, pdf_target, t_0)
    _res = tpc.t_peak_cens_fitter(*args)
    p_best, loss_best, fit_terminates, __ = _res
    assert np.all(np.isfinite(p_best))
    assert loss_best > 0
...
def test_mc_tpeak_cens():
    t_0 = 13.0
    n_gals = int(1e4)
    ran_key = jran.key(0)
    ran_key, m_key, t_key = jran.split(ran_key, 3)
    lgm_obs = jran.uniform(m_key, minval=10.0, maxval=15.0, shape=(n_gals,))
    t_obs = jran.uniform(t_key, minval=2.0, maxval=t_0, shape=(n_gals,))
    args = tpc.DEFAULT_TPCENS_PARAMS, ran_key, lgm_obs, t_obs, t_0
    t_peak_mc_sample = tpc.mc_tpeak_cens(*args)
