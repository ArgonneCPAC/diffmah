...
def test_rolling_index_agrees_with_hard_coded_expectation():
    ...
    index_early = _power_law_index_vs_logt(logt, lgtc_e, k, 10 ** lge_e, 10 ** lgl_e)
    ...
    index_late = _power_law_index_vs_logt(logt, lgtc_l, k, 10 ** lge_l, 10 ** lgl_l)
    assert np.allclose(index_late_correct, index_late, rtol=0.01)

    dmhdt_e, log_mah_e = _calc_halo_history(
        logt, logt0, lgmp_test, lgtc_e, k, 10 ** lge_e, 10 ** lgl_e
    )
    dmhdt_l, log_mah_l = _calc_halo_history(
        logt, logt0, lgmp_test, lgtc_l, k, 10 ** lge_l, 10 ** lgl_l
    )
    ...
    assert np.allclose(log_mah_e_correct, log_mah_e, rtol=0.01)
    ...
    assert np.allclose(log_mah_l_correct, log_mah_l, rtol=0.01)
