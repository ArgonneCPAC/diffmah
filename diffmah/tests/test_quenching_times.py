...
def test_central_qtime_is_monotonic():
    logmp = np.linspace(10, 15, 1000)
    qtimes0 = central_quenching_time(logmp, 0.5)
    qtimes1 = central_quenching_time(logmp, np.zeros_like(logmp) + 0.5)
...
def test_satellite_qtime_is_earlier_than_central_qtime():
    ...
    qtimes1 = satellite_quenching_time(logmp, np.zeros_like(logmp) + 0.5, inftime)
...
def test_inverse_qtimes_default_model():
    for logmp in np.linspace(10, 15, 5):
        for p in np.logspace(-2, -0.01, 6):
            params = dict()
            qtime = central_quenching_time(logmp, p, **params)
            inferred_p = inverse_central_quenching_time(logmp, qtime, **params)
            assert np.allclose(p, inferred_p, rtol=0.01)


def test2_inverse_qtimes_default_model():
    for logmp in np.linspace(10, 15, 5):
        qt_med = central_quenching_time(logmp, 0.5)
        p = inverse_central_quenching_time(logmp, qt_med)
